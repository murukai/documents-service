...
export class PassportService {
  public resourceUrl = this.applicationConfigService.getEndpointFor('api/passports');
  ...
  protected convertDateFromClient(passport: IPassport): IPassport {
    return Object.assign({}, passport, {
      issuedDate: passport.issuedDate?.isValid() ? passport.issuedDate.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.issuedDate = res.body.issuedDate ? dayjs(res.body.issuedDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((passport: IPassport) => {
        passport.issuedDate = passport.issuedDate ? dayjs(passport.issuedDate) : undefined;
      });
    }
    return res;
  }

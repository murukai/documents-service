...
@Component({
  selector: 'jhi-demographic-details',
...
export class DemographicDetailsComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.demographicDetailsService
      ...
      .subscribe(
        (res: HttpResponse<IDemographicDetails[]>) => {
  ...
  trackId(index: number, item: IDemographicDetails): number {
    return item.id!;
  }
  ...
  delete(demographicDetails: IDemographicDetails): void {
    const modalRef = this.modalService.open(DemographicDetailsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.demographicDetails = demographicDetails;
  ...
  protected onSuccess(data: IDemographicDetails[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/demographic-details'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,

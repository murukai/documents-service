...
import com.afrikatek.documentsservice.service.AddressService;
...
/**
 * REST controller for managing {@link com.afrikatek.documentsservice.domain.Address}.
 */
@RestController
@RequestMapping("/api")
public class AddressResource {
    ...
    public ResponseEntity<List<AddressDTO>> getAllAddresses(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("nextofkeen-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where nextOfKeen is null");
            return new ResponseEntity<>(addressService.findAllWhereNextOfKeenIsNull(), HttpStatus.OK);
        }
